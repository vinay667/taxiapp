...
class SelectCabScreenState extends State<SelectCabScreen> {
  ...
  Future<Map<String, dynamic>> fetchServices() async {
    ...
    try {
      ...
      if (fetchResponse['status'] == 'error') {
        ...
        message = fetchResponse['message'];
        List<ServicesResponseModel> list = json.decode(fetchResponse.toString());

        print(list[0].toString()+'uio');

      //  serviceList=List<ServicesResponseModel>.from(fetchResponse.json().map((i) => ServicesResponseModel.fromJson(i)));
  ...
  Future<String> getAccessToken() async
  {
    ...
    token=  prefs.getString('access_token') ?? 'token';
  }
}
